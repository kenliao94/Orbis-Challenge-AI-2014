...
public class PlayerAI implements Player {
    ...
    */
    ...
    //implement the closeness algorithm

    //Close to the top wall assign TOP
    if(closest_wall == "TOP"){
      if (map.isOccupied(curr_x_pos,curr_y_pos-1) == false){
        return PlayerAction.MOVE_UP;
      }
      else if (map.isOccupied(curr_x_pos - 1, curr_y_pos) == false) {
        return PlayerAction.MOVE_LEFT;
      }
      else if (map.isOccupied(curr_x_pos, curr_y_pos + 1) == false){
    ...
    //Close to the left wall assign LEFT
      ...
      if (map.isOccupied(curr_x_pos-1,curr_y_pos) == false){
        return PlayerAction.MOVE_LEFT;
      }
      else if (map.isOccupied(curr_x_pos,curr_y_pos + 1) == false){
        return PlayerAction.MOVE_DOWN;
